...
import { useGetTokenPriceBySymbol } from "@/services/get_token_price_by_symbol";
...
const GetTokenPriceBySymbol = () => {
  ...
  const { getTokenPriceBySymbol } = useGetTokenPriceBySymbol();

  const handleCalculate = async () => {
    if (!ownerAddress || !symbol) {
    ...
    try {
      const response = await getTokenPriceBySymbol(ownerAddress, symbol);
      setResult(response);
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-2xl font-bold text-foreground">Token Price by Symbol</h2>
      </div>
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">Token Symbol</label>
          <input
            type="text"
            value={symbol}
            onChange={(e) => setSymbol(e.target.value)}
            placeholder="Enter token symbol asset"
            className="w-full px-4 py-3 bg-muted border border-border rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 transition-all"
            disabled={loading}
          />
...
export default GetTokenPriceBySymbol;